        ...
        while changed and iterations < 600:
            ...
            for i, c1 in enumerate(clause_list):
                for c2 in clause_list[i + 1:]:
                    for lit in c1:
                        if self.negate(lit) in c2:
                            resolvent = self._resolve(c1, c2, lit)
                            if resolvent is None:
                                continue
                            if len(resolvent) == 0:
                                self.inference_steps += 1
                                return True  # contradiction found → proved
...
class WumpusWorld:
    ...
    def best_move(self):
        ...
        safe_visited = [(nr,nc) for nr,nc in nbrs
                        if (nr,nc) in self.visited and (nr,nc) not in hazard]
        if safe_visited:
...
def step():
    ...
    done = w.status != 'running'
    reveal = done or w.status in ('pit','wumpus','gold','stuck')
    w.step()
